...
function MCACStaffCenterReceptionistPage() {
  ...
  const breadcrumbItems = [
    ...
    {
      title: "Medical Center Reception Member",
      link: "",
    },
  ];
  ...
  const handleCreateMember = () => {
    form
      ...
      .then((values) => {
        ...
        startLoading();
        UserService.postAddNewReceptionMember(values, stopLoading);
        form.resetFields();
        onClose();
      })
      .catch((info) => {
  ...
  const props: UploadProps = {
    name: "file",
    action: `http://localhost:9000/media/upload?email=${email}&userType=mcr&uploadType=profileImage`,
    headers: {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      {!isLoading && (
        <div className="flex-grow px-8">
          <MCSMainGreeting
            title="Medical Center Reception Members"
            titleMemberName=""
          ...
          <div className="w-96 h-12 mb-4">
            <button className="w-96 h-12" onClick={showDrawer}>
              <NormalButtonWithIcon
                ...
                colorType={2}
                link=""
                title="Add a new Medical Center Receptionist"
              />
            </button>
          </div>
...
export default MCACStaffCenterReceptionistPage;
