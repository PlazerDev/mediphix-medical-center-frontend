...
function MCACStaffCenterLabPage() {
  // setting loading
  ...
  const breadcrumbItems = [
    ...
    {
      title: "Medical Center Lab Member",
      link: "",
    },
  ];
  ...
  const handleCreateMember = () => {
    form
      ...
      .then((values) => {
        ...
        startLoading();
        UserService.postAddNewLabMember(values, stopLoading);
        form.resetFields();
        onClose();
      })
      .catch((info) => {
  ...
  const props: UploadProps = {
    name: "file",
    action: `http://localhost:9000/media/upload?email=${email}&userType=mcls&uploadType=profileImage`,
    headers: {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      {!isLoading && (
        <div className="flex-grow px-8">
          <MCSMainGreeting
            title="Medical Center Lab Staff Members"
            titleMemberName=""
          ...
          {/* Drawer :: Add a new MCLS Member */}
          <Drawer
            title="Add a new Medical Center Lab Staff Member"
            onClose={onClose}
            open={open}
          >
            <Form
          ...
          <div className="w-96 h-12 mb-4">
            <button className="w-96 h-12" onClick={showDrawer}>
              <NormalButtonWithIcon
                ...
                colorType={2}
                link=""
                title="Add a new Medical Center Lab Member"
              />
            </button>
          </div>
...
export default MCACStaffCenterLabPage;
